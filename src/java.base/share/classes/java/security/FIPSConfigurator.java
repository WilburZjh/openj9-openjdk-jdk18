...
public final class FIPSConfigurator {
    ...
    static final Debug debug = Debug.getInstance("semerufips");

    // FIPS mode enable check, only support on Linux x64
    private static boolean userEnabledFIPS = false;
    private static boolean isFIPSSupported = false;
    private static boolean shouldEnableFIPS = false;
        ...
        List<String> props = AccessController.doPrivileged(
                new PrivilegedAction<>() {
                    ...
                    public List<String> run() {
                        return List.of(
                                System.getProperty("semeru.fips", "false"),
                                System.getProperty("os.name"),
                                System.getProperty("os.arch"));
                    }
                });
        userEnabledFIPS = Boolean.valueOf(props.get(0));
        isFIPSSupported = "Linux".equalsIgnoreCase(props.get(1))
                && "amd64".equalsIgnoreCase(props.get(2));
    ...
    static boolean configureFIPS(Properties props) {
        ...
        // Check if FIPS is support on this platform
        ...
        try {
            if (shouldEnableFIPS) {
                ...
                StringBuffer disabledAlgorithmsListFIPS = new StringBuffer();
                String disabledAlgorithmsList = (String) props.get("jdk.tls.disabledAlgorithms");
                disabledAlgorithmsListFIPS.append(disabledAlgorithmsList);
                disabledAlgorithmsListFIPS.append(", X25519, X448");
                disabledAlgorithmsListFIPS.append(", SSLv3, TLSv1, TLSv1.1");
                disabledAlgorithmsListFIPS.append(", TLS_CHACHA20_POLY1305_SHA256, "
                ...
                props.put("jdk.tls.disabledAlgorithms", disabledAlgorithmsListFIPS.toString());        
